...
def precompute_embeddings(docs, main_topic, subtopic, api_key):
    ...
    # Chuẩn bị texts
    all_texts = [main_topic, subtopic]
    all_texts.extend([d.get("title", "") + " " + d.get("text", "")[:300] for d in docs])
    ...
    try:
        ...
        for i in range(0, len(all_texts), 100):
            ...
            res = embedding_pool.embed_content(texts=batch, model_name=model_name, primary_provider="openai")
            if not res:
                raise Exception("EmbeddingPool returned None")
            
            for vec in res:
                v_list.append(_normalize(np.array(vec)))
        vectors = v_list
    except Exception as ex:
        import logging
        logging.getLogger(__name__).warning(f"[PrecomputeEmbed] Embedding pool failed: {ex}")
            
    if not vectors or len(vectors) < 2:
        import logging
        logging.getLogger(__name__).warning("[PrecomputeEmbed] All Gemini keys failed. Falling back to OpenAI text-embedding-3-small...")
        if api_key:
            from openai import OpenAI
            try:
                ...
                for i in range(0, len(all_texts), 100):
                    batch = all_texts[i:i+100]
                    resp = o_client.embeddings.create(model="text-embedding-3-small", input=batch)
                    for r in resp.data:
                        v_list.append(_normalize(r.embedding))
                vectors = v_list
            except Exception as oe:
                logging.getLogger(__name__).error(f"[PrecomputeEmbed] OpenAI fallback also failed: {oe}")
                return None, None, []
        else:
            return None, None, []
...
def tao_vector_db(passages: list, api_key: str, start_id: int = 1):
    ...
    # Gemini API thường có giới hạn batch size
    batch_size = 100
    ...
    if to_embed_texts:
        ...
        for i in range(0, len(to_embed_texts), batch_size):
            ...
            try:
                from dich_vu.embedding_pool import embedding_pool
                res = embedding_pool.embed_content(texts=batch_texts, model_name=model_name, primary_provider="openai")
                if not res:
                    raise Exception("EmbeddingPool returned None")
                ...
                for vec in res:
                    v_list.append(_normalize(np.array(vec)))
                batch_vectors = v_list
            except Exception as e:
                logger.warning(f"[Embedding] Embedding pool failed: {e}")
                    
            if not batch_vectors:
                logger.error("[Embedding] All Gemini keys failed for batch.")
            ...
            logger.info(f"[Embedding] Batch {batch_num}/{total_batches} complete ({len(batch_vectors)} vectors)")
            time.sleep(0.1)  # Token Bucket quản lý tốc độ, chỉ cần sleep nhẹ
    else:
...
def tim_kiem_vector(query: str, passages_db: list, api_key: str, top_k: int = 8):
    ...
    try:
        ...
        try:
            from dich_vu.embedding_pool import embedding_pool
            res = embedding_pool.embed_content(model_name=model_name, texts=[query], primary_provider="openai")
            if not res or len(res) == 0:
                raise Exception("EmbeddingPool returned None")
            
            v = np.array(res[0])
            query_vector = _normalize(v)
        except Exception as ex:
            logger.warning(f"[STABILITY] Embedding pool failed for query: {ex}")
                
        if query_vector is None:
            raise RuntimeError("All Gemini keys failed for query embedding.")
    except Exception as e:
