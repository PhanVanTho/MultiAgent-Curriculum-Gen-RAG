    ...
    for api_key in fallback_keys:
        ...
        for attempt in range(2):
            try:
                ...
                logger.info(
                    f"[GeminiSupervisor] Chapter '{chapter_content.get('title')}': "
                    f"status={normalized['status']}, model={fallback_model} (Fallback)"
                )
                return normalized
                ...
                if "429" in str(e) or "RESOURCE_EXHAUSTED" in str(e):
                    logger.warning(f"[GeminiSupervisor] Fallback 429 hit (Key ...{api_key[-4:]}). Attempt {attempt+1}. Backoff {delay}s...")
                    time.sleep(delay)
                    delay *= 2
                    continue
                break
    ...
    for api_key in fallback_keys:
        ...
        for attempt in range(2):
            try:
                ...
                logger.info(f"[GeminiSupervisor-Outline] Fallback successful with {fallback_model} | status={result.get('status')}")
                return result
            except Exception as e:
                last_error = e
                if "429" in str(e) or "RESOURCE_EXHAUSTED" in str(e):
                    logger.warning(f"[GeminiSupervisor-Outline] Fallback 429 key ...{api_key[-4:]}. Backoff {delay}s...")
                    time.sleep(delay)
                    delay *= 2
                    continue
                break
        ...
        for api_key in fallback_keys:
            ...
            for attempt in range(2):
                try:
                    ...
                    last_error = e
                    if "429" in str(e) or "RESOURCE_EXHAUSTED" in str(e):
                        logger.warning(f"[GeminiSupervisor-QuyMo] Fallback 429 key ...{api_key[-4:]}. Backoff {delay}s...")
                        time.sleep(delay)
                        delay = min(delay * 2, 2)  # Cap at 2s
                        continue
                    break
