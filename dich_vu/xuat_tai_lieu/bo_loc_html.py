...
def clean_for_reportlab(html_text):
    ...
    # 2.5 Markdown Tag Mapping (trong HTML được sinh từ parse_markdown)
    for tag in soup.find_all("strong"):
        tag.name = "b"
    for tag in soup.find_all("em"):
        tag.name = "i"
    for tag in soup.find_all("code"):
        # ReportLab ko hỗ trợ code, map qua font monospace hoặc b
        tag.name = "b"

    # 3. Block-level handling: Tránh dính chữ khi unwrap các thẻ khối
    BLOCK_TAG_LIST = ["div", "p", "section", "article", "h1", "h2", "h3", "h4", "h5", "h6", "li", "pre", "ul", "ol", "blockquote"]
    for block_tag in soup.find_all(BLOCK_TAG_LIST):
        # Sử dụng delimiter đặc biệt thay vì \n để tránh cắt ngang thẻ inline (b, i) khi split
        block_tag.insert_before("|||BLOCK|||")
        
        # Nếu là thẻ li, prepend dấu bullet
