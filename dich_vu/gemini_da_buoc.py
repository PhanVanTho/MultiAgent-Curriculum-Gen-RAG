...
# Global Semaphore: Giới hạn tối đa 2 request Gemini text-gen đồng thời
_GEMINI_TEXT_SEMAPHORE = threading.Semaphore(2)

# V41: Global Project-Level Rate Limiter (Token Bucket)
# Dựa trên dữ liệu thực tế từ Google AI Studio:
#   gemini-2.5-flash:      5 RPM,  20 RPD
#   gemini-2.5-flash-lite: 10 RPM, 20 RPD
#   gemini-3.1-flash-lite: 15 RPM, 500 RPD  ← TỐI ƯU NHẤT
#   gemini-3-flash:        5 RPM,  20 RPD
# Đặt 8 RPM (an toàn cho model thấp nhất, ceiling cho mixed usage)
_GEMINI_PROJECT_RPM = 8

class _ProjectRateLimiter:
    """Token Bucket rate limiter cấp độ PROJECT.
    Tất cả key và thread đều chia sẻ 1 bucket duy nhất.
    """
    def __init__(self, rpm: float):
        self.capacity = rpm
        self.tokens = rpm
        self.refill_rate = rpm / 60.0  # tokens/giây
...
_GEMINI_PROJECT_BUCKET = _ProjectRateLimiter(_GEMINI_PROJECT_RPM)

# V2.1 Circuit Breaker: Lưu trạng thái Cooldown của các API key (tránh Thundering Herd 429)
...
def _get_gemini_response(prompt: str, api_keys: list, model_name: str = CauHinh.SUPERVISOR_MODEL_LITE):
    ...
    for attempt in range(max_retries):
        ...
        # V2.1: Circuit Breaker - Lọc bỏ các key đang bị Cooldown (429)
        available_keys = [k for k in shuffled_keys if time.time() > _GEMINI_KEY_COOLDOWN.get(k, 0)]
        
        if not available_keys:
            logger.warning(f"[Gemini Circuit Breaker] Tất cả {len(api_keys)} keys đều đang bị Quota Cooldown. Fallback ngay lập tức!")
            break # Cắt đứt vòng lặp attempt, nhảy thẳng xuống khối OpenAI Fallback
            
        for current_key in available_keys:
            # Global Semaphore: Chờ cho tới khi có slot trống (tối đa 2 concurrent)
            _GEMINI_TEXT_SEMAPHORE.acquire()
            client = genai.Client(api_key=current_key)
            try:
                # V41: Project-level rate limit (chờ token từ bucket chung)
                if not _GEMINI_PROJECT_BUCKET.acquire(timeout=60.0):
                    logger.warning(f"[Gemini] Project rate limit timeout. Falling back...")
                    _GEMINI_TEXT_SEMAPHORE.release()
                    break
                
                # Jitter tăng cường: Phân tán tải để không bị Google đánh là spam
                ...
                if any(x in error_str for x in ["429", "RESOURCE_EXHAUSTED", "quota"]):
                    ...
                    _GEMINI_KEY_COOLDOWN[current_key] = time.time() + 60 # Cooldown key này trong 60 giây
                    time.sleep(random.uniform(0.5, 1.0))  # Giảm thời gian chờ chết, chuyển key khác ngay
                    continue
                elif "403" in error_str or "PERMISSION_DENIED" in error_str:
                    logger.error(f"[Gemini] Key ...{current_key[-4:]} is DEAD (403 PERMISSION_DENIED). Removing from pool!")
                    _GEMINI_KEY_COOLDOWN[current_key] = time.time() + 86400 # Cooldown 24h
                    continue
                    ...
                    time.sleep(1)
                    continue # V39.1: Không break, tiếp tục thử key khác trong mảng
        if attempt < max_retries - 1:
            time.sleep(random.uniform(4.0, 6.0) + attempt * 5)
    
    # === OPENAI FALLBACK ===
    # Tất cả Gemini keys đã kiệt sức → chuyển sang OpenAI gpt-4o-mini
    logger.warning(f"[Gemini→OpenAI] All Gemini keys exhausted for {model_name}. Falling back to OpenAI gpt-4o-mini...")
    try:
        ...
        resp = o_client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "You are a helpful assistant. Respond in the same language as the user's prompt."},
                {"role": "user", "content": prompt}
            ],
            temperature=0.7,
        )
        result = resp.choices[0].message.content or ""
        logger.info(f"[Gemini→OpenAI] Fallback successful. Response length: {len(result)} chars.")
        return result
    except Exception as oe:
        logger.error(f"[Gemini→OpenAI] OpenAI Fallback also failed: {oe}")
        raise last_error  # Raise lỗi Gemini gốc nếu OpenAI cũng thất bại
...
{{
  ...
  "outline": [
    {{
      ...
      "sections": [
        {{"title": "Tên mục", "subsections": ["Tên mục con 1", "Tên mục con 2"], "recommended_pids": ["pid1", "pid2"]}}
      ]
    }}
  ]
}}
        ...
        for current_key in api_keys:
            ...
            try:
                ...
                if "429" in error_str or "RESOURCE_EXHAUSTED" in error_str or "quota" in error_str.lower():
                    logger.warning(f"[WARN] Gemini Step 1: Quota exceeded with key ...{current_key[-4:]}. Switching key...")
                    continue
                else:
                    logger.warning(f"[WARN] Gemini Step 1 error: {e}")
                    time.sleep(2)
                    break
        else:
            if attempt < max_retries - 1:
                logger.warning(f"[WARN] All Gemini keys exhausted for Step 1. Sleeping 60s...")
                time.sleep(60)
            else:
                pass # Continue to fallback outside the loop
    
    # === OPENAI FALLBACK ===
    logger.warning(f"[Gemini→OpenAI] Step 1: All Gemini keys exhausted. Falling back to OpenAI gpt-4o-mini...")
    try:
        from openai import OpenAI
        o_client = OpenAI(api_key=CauHinh.OPENAI_API_KEY)
        resp = o_client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "You must return pure JSON."},
                {"role": "user", "content": prompt}
            ],
            temperature=0.7,
        ...
        logger.error(f"[Gemini→OpenAI] Step 1 Fallback failed: {oe}")
        if last_error:
            raise last_error
        else:
            raise oe
        ...
        for current_key in api_keys:
            ...
            try:
                ...
                return json.loads(_tach_json(text))
                 ...
                 error_str = str(e)
                 if "429" in error_str or "RESOURCE_EXHAUSTED" in error_str or "quota" in error_str.lower():
                     logger.warning(f"[WARN] Gemini Fact-check: Quota exceeded with key ...{current_key[-4:]} for chapters {chapters_titles}. Switching key...")
                     continue
                 else:
                     logger.warning(f"[WARN] Gemini Fact-check error (Chapters {chapters_titles}): {e}")
                     time.sleep(2)
                     break
        else:
             if attempt < max_retries - 1:
                logger.warning(f"[WARN] All Gemini keys exhausted for Fact-check. Sleeping 60s...")
                time.sleep(60)
             else:
                pass # Continue to fallback
                
    # === OPENAI FALLBACK ===
    logger.warning(f"[Gemini→OpenAI] Fact-check: All Gemini keys exhausted. Falling back to OpenAI gpt-4o-mini...")
    try:
        from openai import OpenAI
        o_client = OpenAI(api_key=CauHinh.OPENAI_API_KEY)
        resp = o_client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "You must return pure JSON array."},
                {"role": "user", "content": prompt}
            ],
            temperature=0.2,
        )
        ...
        logger.error(f"[Gemini→OpenAI] Fact-check Fallback failed: {oe}")
        if last_error:
            raise last_error
        else:
            raise oe
...
def gemini_critic_agent(topic: str, doc_title: str, doc_text: str, api_keys: list = None, aliases: list = None) -> dict:
    """
    AGENT 2: THE CRITIC (Thẩm định viên LLM)
    Đọc văn bản và chấm điểm độ chính xác học thuật dựa trên chủ đề.
    V36.4: Nhận thêm aliases từ Truth Seed để không đánh rớt bài viết hợp lệ
    khi người dùng nhập tên gọi khác (ví dụ: 'Tết đoàn viên' = 'Tết Trung thu').
    """
    if not api_keys: api_keys = CauHinh.GEMINI_API_KEYS
    ...
    # V36.4: Xây dựng alias context để Critic hiểu các tên gọi khác nhau
    alias_context = ""
    if aliases:
        alias_list = ', '.join(aliases[:10])
        alias_context = f"""\nIMPORTANT ALIAS CONTEXT: The topic \"{topic}\" is also known by these names: [{alias_list}].
An article about ANY of these aliases IS RELEVANT to the topic. Do NOT reject an article just because it uses a different name for the same subject.\n"""
...
EVALUATION RUBRIC:
1. Relevance: Is the excerpt factually related to the core subject of "{topic}" (or any of its known aliases/alternate names)? It does NOT need to cover the complex academic angle perfectly, as long as it provides foundational facts, history, or concepts about the subject.
...
"""
    try:
        ...
        logger.warning(f"[Agent Critic] Gemini failed ({e}). Falling back to OpenAI gpt-4o-mini...")
        from openai import OpenAI
        try:
            o_client = OpenAI(api_key=CauHinh.OPENAI_API_KEY)
            resp = o_client.chat.completions.create(
            ...
            return {
                "is_approved": bool(parsed.get("is_approved", False)),
                "confidence_score": int(parsed.get("confidence_score", 0)),
                "reason": str(parsed.get("reason", "OpenAI Fallback"))
            }
        except Exception as oe:
            logger.error(f"[Agent Critic] OpenAI Fallback failed: {oe}")
