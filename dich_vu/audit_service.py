...
class ScholarlyAuditEngine:
    ...
    def calculate_vector_scores(self, fact_mappings: list) -> list:
        ...
        try:
            ...
            try:
                ...
                res = embedding_pool.embed_content(texts=all_texts, model_name=model_name, primary_provider="gemini")
                if not res:
                    raise RuntimeError("EmbeddingPool returned None")
                
                vectors = []
                for vec in res:
                    vectors.append(_normalize(np.array(vec)))
                embeddings = vectors
            except Exception as ex:
                logger.warning(f"[AuditEngine] Embedding pool failed: {ex}")
                raise RuntimeError("All Gemini keys failed for embedding.")
    ...
    def gemini_soft_audit(self, fact_mappings: list, chu_de: str) -> list:
        ...
        for i, m in enumerate(fact_mappings):
            if m.get("vector_score", 1.0) < 0.55:
...
QUY TẮC NGHIÊM NGẶT:
1. TRỰC TIẾP & DIỄN ĐẠT LẠI: Claim phải được suy ra từ Span. Chấp nhận diễn đạt lại (paraphrasing): Nếu Claim diễn đạt lại ý của Span bằng từ ngữ học thuật khác nhưng không làm sai lệch ý nghĩa cốt lõi, hãy đánh giá là Hợp lệ (YES).
2. KHÔNG SUY DIỄN: Nếu Span không nói, dù Claim nghe có vẻ đúng thực tế, vẫn phải đánh dấu là "unsupported".
3. KHÔNG DÙNG KIẾN THỨC NGOÀI: Chỉ dùng dữ liệu trong Span để đối soát. Không bắt bẻ câu chữ quá khắt khe, miễn thông tin khoa học/sự kiện là chính xác.
4. PHÂN LOẠI LỖI:
...
{{
  "audit_results": [
    ...
    def batch_audit_chunks(self, chunks: list, tieu_de: str) -> list:
        """
        V28: Chunked Batch Critic - Đánh giá gộp nhiều chương cùng lúc.
        Mô phỏng Quota Guard và Tiered Audit để bảo vệ ngân sách.
        """
        if getattr(QuotaManager, 'api_calls_today', 0) >= 15:
            logger.warning(f"[QuotaGuard] Đã vượt ngưỡng 15 requests/ngày. Bypass Critic.")
            return chunks
            
        audited_chunks = []
        for chap in chunks:
            if not hasattr(QuotaManager, 'api_calls_today'):
                QuotaManager.api_calls_today = 0
            QuotaManager.api_calls_today += 1
            ...
            if content_length < 100:
                logger.warning(f"[BatchCritic] Chương '{title}' quá ngắn. Bỏ qua.")
                audited_chunks.append(chap)
                continue
                
            logger.info(f"[BatchCritic] Chương '{title}' Passed Quality Gate.")
            audited_chunks.append(chap)
            
        return audited_chunks
